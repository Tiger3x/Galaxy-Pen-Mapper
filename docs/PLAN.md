...
**Current P005A milestone:** validate the [conservative pen-pattern probe](P005A-PEN-SIGNATURE.md) prospectively with separate S Pen and PW500 tip captures. The S Pen reference capture itself has short `0x28/0x2C` intervals, so a single report cannot identify the physical pen. P002.6 labels only sustained, contact-supported PW500-compatible evidence; hover-only and mixed evidence remain inconclusive. If on-device validation exposes false identifications, adjust or reject user-mode identification before any substitution experiment.

**Next gate after P005A:** investigate whether the original pen stream can be isolated in a controlled target application. Do not inject a replacement stream before a no-duplicate mechanism is demonstrated. Only then consider a degraded-pressure experiment; the saturated Raw samples do not contain the lost force information.
