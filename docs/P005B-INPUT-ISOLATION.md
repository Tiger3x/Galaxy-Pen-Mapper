...
2. Per the user's later decision, use a [manual tray-controlled PW500 mode](P007-MANUAL-MODE.md), not automatic pen identification. Design a fail-open pass-through: off, startup, errors, control-app exit and filter shutdown must leave the original input unchanged. S Pen behavior while the PW500 mode is on is outside the intended use.
