...
These are empirical criteria for a controlled test, not proof that every possible S Pen action is excluded. In replay of existing captures, the S Pen session produced no PW500-candidate rows; the initial PW500 session produced no S-Pen-pattern rows. A PW500 hover-only capture correctly remained inconclusive. Prospective hardware tests would be useful before using this label to control live input, but are not needed for the architectural isolation review.

## Optional future tip-only captures
...
These captures are **not requested now**. The existing data suffices for P005B. Before any future live input replacement, success would mean the S Pen never receives a PW500-candidate label, the PW500 reaches one during normal tip contact, and neither depends on a side-button action. If the result fails or stays inconclusive, stop and inspect the conflicting Raw rows.

## Gate after this test

Even a correct pattern label only observes input. It does not prevent the original erroneous contact or pressure from reaching another application. The next independent P005 gate is the [input-isolation review](P005B-INPUT-ISOLATION.md). It found no documented, reliable general-desktop user-mode replacement mechanism in the current design. Synthetic pen injection and pressure transformation remain disabled.
